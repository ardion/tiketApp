...
module.exports = {
  ...
  checkgetpriceModel: (arr) => {
    return new Promise((resolve, reject) => {
      db.query(`SELECT * FROM table_price WHERE order_class='${arr[0]}' AND passangger='${arr[1]}' AND city_destination='${arr[2]}' AND city_depature='${arr[3]}' AND times_flight= '${arr[4]}'`, (error, result) => {
        // SELECT * FROM table_price WHERE id_plane=1 AND class="economy" AND passangger="adult" AND city_destination="jakarta" AND city_depature="surabaya" AND times_flight="pagi"
        // console.log(result)
        if (!error) {
          resolve(result)
