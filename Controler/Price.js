const {
  checkpriceModel, postPaymentModel, postOrderModel, checkgetpriceModel
} = require('../Model/Price')
  ...
  getDataPrice: async (request, response) => {
    try {
      ...
      if (nadult >= 0 && nchild >= 0) {
        ...
        const checkDataPrice = await checkgetpriceModel([order_class, passangger, city_destination, city_depature, times_flight])
        ...
        const checkDataPrice2 = await checkgetpriceModel([order_class, passangger, city_destination, city_depature, times_flight])
